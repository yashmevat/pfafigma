    ...
    <section className="max-w-5xl mx-auto px-6 py-12 bg-white">
      ...
      <div className="flex flex-col space-y-6">
        {assessments.map((item, index) => (
          <div
            key={item.id}
            className="flex items-start gap-4 bg-white rounded-lg p-5 hover:shadow-md transition-shadow duration-200"
          >
            {/* Number Badge */}
            <div
              className={`flex items-center justify-center w-8 h-8 rounded-md font-semibold ${colors[index]} flex-shrink-0`}
            >
              {item.id}
            </div>
